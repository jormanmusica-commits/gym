...
const NadaTracker: React.FC<{ dayName: string; }> = ({ dayName }) => {
                    ...
                    <div className="w-full px-4 mb-1">
                        <div className="flex items-center justify-between w-full bg-white/5 pl-4 pr-2 py-1.5 rounded-full border border-white/10 shadow-lg shadow-black/20">
                            <div className="flex items-center gap-2 overflow-hidden">
                                ...
                                <span className="text-gray-300 font-bold text-sm whitespace-nowrap">{formatDisplayDate(date)}</span>
                            </div>
                            {activeSede && (
                                <div className="flex items-center gap-3 flex-shrink-0">
                                    <div className="h-4 w-px bg-white/10"></div>
                                    <span className={`${getSedeColor(activeSede)} text-[9px] font-black px-2.5 py-1.5 rounded-full flex items-center gap-1 font-mono uppercase tracking-widest leading-none shadow-sm`}>
                ...
                {hasMetrics && (
                  <div className="mt-4 pt-4 border-t border-gray-700/50">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-3 text-sm">
                      {metrics.speed?.trim() && <div className="flex items-center gap-2"><BarChart4 className="w-5 h-5 text-cyan-400 flex-shrink-0"/><div><p className="text-xs text-gray-400">Velocidad</p><p className="font-semibold text-white">{formatMetric(metrics.speed, 'KM/H')}</p></div></div>}
                      {metrics.distance?.trim() && <div className="flex items-center gap-2"><BarChart4 className="w-5 h-5 text-cyan-400 flex-shrink-0"/><div><p className="text-xs text-gray-400">Distancia</p><p className="font-semibold text-white">{formatMetric(metrics.distance, metrics.distanceUnit || 'KM')}</p></div></div>}
                      {metrics.incline?.trim() && <div className="flex items-center gap-2"><BarChart4 className="w-5 h-5 text-cyan-400 flex-shrink-0"/><div><p className="text-xs text-gray-400">Inclinación</p><p className="font-semibold text-white">{formatMetric(metrics.incline, '%')}</p></div></div>}
